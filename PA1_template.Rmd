...
```{r weekdayfunc}
day <- factor(weekdays(steps_filledin_by_date$date) %in% c("Saturday","Sunday"), 
          labels=c("weekday","weekend"), ordered=FALSE)
steps_filledin_by_date$day <- day
```
...
```{r weekdayplot}
#fix 
qplot(interval,total_steps, data=steps_filledin_by_date, geom="line",
      xlab="Interval (5min steps)",
      facets=day~.)
```